...
# 通用财经模拟标题（用 {keyword} 拼接）
_GENERIC_TEMPLATES = [
    # 利多
    "{keyword}市场情绪乐观，需求预期向好",
    "{keyword}供给端收紧，价格有望上行",
    "{keyword}库存下降，机构上调需求展望",
    "资金持续流入，{keyword}价格走强",
    # 利空
    "{keyword}需求走弱，价格承压回落",
    "{keyword}库存高企，市场担忧供应过剩",
    "宏观经济数据疲软，{keyword}遭遇抛售",
    "市场风险偏好下降，{keyword}价格走低",
    # 中性
    "{keyword}市场情绪波动，投资者观望",
    "{keyword}价格窄幅震荡，等待方向指引",
    "分析师：{keyword}短期方向不明，关注后续数据",
    "{keyword}多空因素交织，市场保持谨慎",
]


def _select_templates(keyword: str) -> list[str]:
    """根据关键词返回对应的模拟标题模板。"""
    kw = keyword.lower()
    if "原油" in keyword or "oil" in kw or "crude" in kw:
        return _OIL_TEMPLATES
    if "黄金" in keyword or "gold" in kw:
        return _GOLD_TEMPLATES
    # 其他：通用财经模板，拼接 keyword
    return [t.format(keyword=keyword) for t in _GENERIC_TEMPLATES]


def generate_mock_news(keyword: str, count: int = 10) -> list[dict]:
    """生成模拟新闻数据，用于真实请求失败/为空时的降级。

    根据关键词返回对应主题（原油/黄金/通用），覆盖利多、利空、中性三类。
    ...
    Returns:
        模拟新闻记录列表。
    """
    templates = _select_templates(keyword)
